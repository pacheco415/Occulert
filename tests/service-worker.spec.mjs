...
test('the service worker installs its offline shell', async ({ page, context, browserName }) => {
  ...
  const refreshedScript = await page.evaluate(async () => {
    const cache = await caches.open('occulert-v54');
    await cache.put('/driver-app.v60.js', new Response('stale-driver-script'));
    return fetch('/driver-app.v60.js').then(response => response.text());
  });
  ...
  await expect.poll(() => page.evaluate(async () => {
    const cache = await caches.open('occulert-v54');
    const response = await cache.match('/driver-app.v60.js');
  ...
  try {
    const offlineShell = await page.evaluate(async () => {
      const [response, driverScript] = await Promise.all([fetch('/app.html'), fetch('/driver-app.v60.js')]);
