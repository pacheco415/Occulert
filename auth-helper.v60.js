...
(function(){
  ...
  async function saveBackendProfile(profile,context){
  ...
  async function signInEmail(email,password,mode,extra){
    ...
    if(!backend)throw new Error('Account settings could not load. Reload and try again.');
    var attempt=backend.beginAuthAttempt();
    ...
    var result=mode==='signup'?await window.OcculertBackend.signUp(email,password,attempt):await window.OcculertBackend.signIn(email,password,attempt);
    if(!result.ok){
    ...
    var context=backend.completedAuthContext(attempt);
    ...
    return saveBackendProfile(mergeUserIntoProfile(currentUser,extra),context);
  }
  async function signInPasskey(){
    if(!window.OcculertPasskeys)throw new Error('Passkey sign-in is not available on this page.');
    var attempt=window.OcculertBackend.beginAuthAttempt();
    var verified=await window.OcculertPasskeys.signIn(attempt);
    var context=window.OcculertBackend.completedAuthContext(attempt);
  ...
  async function signOut(){
    ...
    try{if(window.OcculertPasskeys)Promise.resolve(window.OcculertPasskeys.signOutLocal()).catch(function(){});}catch(e){}
    try{localStorage.removeItem('occulert-auth-user');}catch(e){}
    var profile=getProfile();
    if(profile){profile.authenticated=false;profile.cloudProfile=false;try{saveProfile(profile);}catch(e){}}
    currentUser=null;
  }
  async function onAuth(cb){
    var backend=null,context=window.OcculertBackend&&window.OcculertBackend.captureAuthContext();
    try{
    ...
    var stored=window.OcculertBackend&&window.OcculertBackend.captureAuthContext().auth;
    if(backend&&(!stored||stored.access_token!==session.access_token||stored.refresh_token!==session.refresh_token||!stored.user||stored.user.id!==backend.id))return;
    if(!backend&&stored&&context&&!window.OcculertBackend.isAuthContextCurrent(context))return;
