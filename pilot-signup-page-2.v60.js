...
document.getElementById('pilotForm').noValidate=true;
...
const validatedFieldIds=['name','company','email','plan','timeline','goal'];
function clearFieldErrors(){validatedFieldIds.forEach(id=>document.getElementById(id)?.removeAttribute('aria-invalid'))}
function showFieldError(id,message){const field=document.getElementById(id),error=document.getElementById('error');field?.setAttribute('aria-invalid','true');error.textContent=message;error.style.display='block';field?.focus()}
validatedFieldIds.forEach(id=>{const field=document.getElementById(id);['input','change'].forEach(eventName=>field?.addEventListener(eventName,()=>field.removeAttribute('aria-invalid')))});
async function saveLead(event){
  ...
  success.style.display='none';error.style.display='none';clearFieldErrors();
  ...
  if(!planMatchesInterest(lead.plan)){showFieldError('plan','Choose a plan that matches this trial or rollout request.');return}
  if(!lead.timeline){showFieldError('timeline','Choose your desired start window.');return}
  if(!lead.goal){showFieldError('goal','Choose your primary operating goal.');return}
  leadSubmissionPending=true;btn.disabled=true;btn.textContent='Sending...';
  const controller=new AbortController();let timeoutId;
  const deadline=new Promise((_,reject)=>{timeoutId=setTimeout(()=>{controller.abort();reject(new Error('request_timeout'))},8000)});
  try{
    const request=(async()=>{const res=await fetch('/api/pilot-leads',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(lead),signal:controller.signal}),result=await res.json().catch(()=>({}));return{res,result}})();
    const {res,result}=await Promise.race([request,deadline]);
    if(!res.ok||result.stored!==true)throw new Error(result.error||'request_not_stored');
    success.textContent=freeTrialInterest?'Free trial request saved securely. No credit card was collected, no automatic renewal was created, and no contact copy was kept in this browser.':rolloutInterest?'Rollout conversation requested and saved securely. No contact copy was kept in this browser.':'Fleet request sent and saved securely. No contact copy was kept in this browser.';success.style.display='block';success.setAttribute('tabindex','-1');success.focus();
    ['name','role','company','email','phone','message','website'].forEach(id=>{const field=document.getElementById(id);if(field.value.trim()===lead[id])field.value=''});
  }catch(e){error.textContent='We could not confirm that your request was saved. Your entries are still here. Please try again later or email fleet@occulert.com.';error.style.display='block';error.setAttribute('tabindex','-1');error.focus()}
  finally{clearTimeout(timeoutId);leadSubmissionPending=false;btn.disabled=false;btn.textContent=buttonLabel}
}
