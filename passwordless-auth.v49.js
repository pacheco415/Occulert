...
(function () {
  ...
  async function client() {
    if (!clientPromise) {
      clientPromise = Promise.all([
        ...
        if (!config || !config.url || !config.anonKey) throw new Error('Account settings could not load. Please retry.');
      ...
      }).catch(error => { clientPromise = null; retryClient = true; throw error; });
    }
  ...
  async function consumeRedirect() {
    ...
    const adopted = window.OcculertBackend.adoptSession({ ...data.session, user });
