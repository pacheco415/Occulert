(function (root) {
  ...
  var state = { model: null, summary: null, userId: '', lastSuccess: 0, failures: 0, loading: false, timer: 0, nextRefresh: 0, days: 30, authVersion: 0, refreshPending: false, connection: 'checking' };
  ...
  function updateClock() {
    ...
    if (!state.userId) detail += 'A signed-in fleet owner is required.';
  ...
  async function refresh() {
    ...
    try {
      ...
      updateClock();
      var result = await backend.getFleetSummary({ includeEvents: false });
      if (requestVersion !== state.authVersion) return;
