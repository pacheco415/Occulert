...
| Task | Priority | Status |
|------|----------|--------|
| Unified confidence model fusing camera + earbud + watch signals | ★ | Observation-only local aggregation is implemented in source for camera state/head-nod counts, compatible headphone motion/head-nod counts, Apple Watch availability, and bounded co-occurrences. It stores no raw sensor timeline, does not affect scoring or alerts, and is excluded from cloud sync and exports. Validation and any confidence model remain future work |
...
| 3 · Fusion + Fleet | Multi-signal model | Unified confidence engine | Partial (protected fleet history and dashboard performance work implemented; observation-only local fusion diagnostics implemented in source, with validation and scoring integration not started) |
