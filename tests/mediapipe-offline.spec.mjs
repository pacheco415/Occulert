...
for (const upgrade of [false, true]) {
  test(`${upgrade ? 'failed upgrade preserves v47' : 'failed fresh install stays inactive'} until the complete runtime is verified`, async ({ page }) => {
    ...
    const server = createServer((request, response) => {
      ...
      const brokenAsset = runtimeRoot + 'face_mesh_solution_wasm_bin.wasm';
      if (state.failure && path === brokenAsset) {
        response.statusCode = state.failure === 'missing' ? 503 : 200;
        response.end('invalid runtime bytes');
        return;
      }
      const file = state.old && path === '/sw.js' ? resolve('tests/fixtures/sw-v47.js')
        : state.old && path === '/app.html' ? resolve('tests/fixtures/app-v47.html')
    ...
    try {
      ...
      if (upgrade) {
        ...
        await expect.poll(() => page.evaluate(() => caches.has('occulert-v47'))).toBe(true);
        state.old = false;
        state.failure = 'corrupt';
      }
      expect(await register(upgrade)).toBe('redundant');
      const keys = await page.evaluate(() => caches.keys());
      expect(keys).not.toContain('occulert-v48');
      if (upgrade) {
        expect(keys).toContain('occulert-v47');
        expect(await page.evaluate(async () => (await (await caches.open('occulert-v47')).match('/app.html')).text())).toContain('/driver-app.v47.js');
      } else {
      ...
      const cached = await page.evaluate(async () => (await (await caches.open('occulert-v48')).keys()).map(request => new URL(request.url).pathname));
      for (const file of Object.keys(manifest.files)) expect(cached).toContain(runtimeRoot + file);
      expect(await page.evaluate(() => caches.has('occulert-v47'))).toBe(false);
      state.offline = true;
