...
assertIncludes("supabase/migrations/20260926010000_atomic_fleet_invitation_creation.sql", "invitation_rate_limited", "fleet invitation creation must enforce a transactional abuse limit");
...
assertIncludes("login.html", "src=\"/passkey-auth.v60.js\"", "login must load the passkey client");
assertIncludes("passkey-auth.v60.js", "experimental: { passkey: true }", "passkey support must be explicitly enabled in the Supabase client");
assertIncludes("passkey-auth.v60.js", "signInWithPasskey", "passkey sign-in must use the Supabase WebAuthn implementation");
assertIncludes("auth-helper.v60.js", "signInPasskey:signInPasskey", "the login helper must adopt authenticated passkey sessions");
assertIncludes("auth-helper.v60.js", "await window.OcculertBackend.getSession()", "account-state rendering must validate or refresh the stored session before showing signed-in controls");
...
assertIncludes("account-page-2.v60.js", "OcculertBackend.updateEmail", "account email changes must go through Supabase Auth");
assertIncludes("account-page-2.v60.js", "OcculertBackend.updatePassword", "account password changes must go through Supabase Auth");
assertIncludes("account.html", "id=\"registerPasskeyBtn\"", "signed-in account settings must offer passkey enrollment");
assertIncludes("account-page-2.v60.js", "data-passkey-action=\"remove\"", "account settings must allow users to revoke their own passkeys");
...
assertIncludes("passkey-auth.v60.js", "client.auth.registerPasskey", "passkey registration must use the authenticated Supabase ceremony");
assertIncludes("passkey-auth.v60.js", "client.auth.passkey.delete", "passkey deletion must use the authenticated Supabase account API");
assertIncludes("passkey-auth.v60.js", "signOut({ scope: \"local\" })", "sign-out must clear the passkey SDK's current browser session without signing out other devices");
assertIncludes("sw.js", "'/passkey-auth.v60.js'", "the service worker must keep the passkey client network-only");
...
assertIncludes("account-page-2.v60.js", "await backend.getFleet()", "account access must verify server-owned fleet access");
assertIncludes("account-page-2.v60.js", "Verified role", "account summaries must label the server-authoritative role");
