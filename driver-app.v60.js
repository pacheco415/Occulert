// Complete initialization is required before any monitoring entry point.
function startupAllowsMonitoring(){return typeof window.OcculertStartup?.isReady==='function'&&window.OcculertStartup.isReady()===true}
...
async function findCameraChoices(){
  if(!startupAllowsMonitoring())return;
...
drawEyes(lm,fatigue>=60?'#ff3344':fatigue>=35?'#ffaa00':'#00ff88');render(ear);if(!calibrating&&confidence>=45&&fatigue>=80)trigger('Fatigue');else if(!calibrating&&confidence>=45&&turnedSince&&Date.now()-turnedSince>6500)trigger('Distraction')}async function initCloud(){if(!startupAllowsMonitoring()||!cloudConsent.checked){cloudReady=false;syncEl.textContent='LOCAL';return}try{const configured=window.OcculertBackend&&await window.OcculertBackend.isConfigured();const user=configured&&window.OcculertBackend.currentUser();cloudReady=!!user;syncEl.textContent=cloudReady?'ON':'LOCAL';log(cloudReady?'Protected cloud sync ready':configured?'Sign in before enabling cloud sync':'Cloud sync is not configured - using local mode')}catch(e){cloudReady=false;syncEl.textContent='LOCAL';log('Cloud sync unavailable - using local mode')}}
...
let localSessionId='';
async function requestSessionWakeLock(expectedSessionId){
  ...
  try{
    ...
    if(running&&localSessionId===expectedSessionId)log('Wake lock unavailable - keep screen unlocked');
  }
}
function saveLocalSessionHistory(payload){
  if(!sessionStart||!localSessionId)return null;
  try{
    const existing=JSON.parse(localStorage.getItem('occulert-session-history')||'[]');
    if(!Array.isArray(existing))throw new Error('Local history is not a list.');
    ...
    const rows=existing.filter(row=>row&&typeof row==='object'&&row.id!==localSessionId&&row.sessionId!==localSessionId);
    localStorage.setItem('occulert-session-history',JSON.stringify([saved,...rows].slice(0,50)));
    return saved;
  }catch(error){
...
// Final synchronous statement: partial scripts and hoisted functions cannot signal readiness.
if(window.OcculertStartup)window.OcculertStartup.ready(Object.freeze({version:'v60',start,stop,findCameraChoices,initModel}));
